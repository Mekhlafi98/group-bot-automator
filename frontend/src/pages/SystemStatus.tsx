...
const SystemStatus: React.FC = () => {
    ...
    const saveMutation = useMutation({
        mutationFn: async () => {
            if (editItem) {
                const res = await api.put(`/system-status/${editItem._id}`, form);
                return res.data;
            } else {
                const res = await api.post('/system-status', form);
                return res.data;
            }
        },
        onSuccess: () => {
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id: string) => {
            await api.delete(`/system-status/${id}`);
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["system-status"] });
        },
    ...
    const checkMutation = useMutation({
        mutationFn: async (id: string) => {
            await api.post(`/system-status/${id}/check`);
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["system-status"] });
        },
